...
private:
  ...
  //HELPER FUNCTIONS
  /*Helper function that traverses the tree recursively to build the encoding table, 
  mapping characters to their "bit string representations" of dots "." and carets "^".
  Takes as arguments a current node (starting from root), a string concatenating the above characters, 
  and the address of the encoding table we are building up.*/
  void TreeTraversalRecursive(shared_ptr<freq_info> currNode, string temp, map<char, string> &enc_table);
